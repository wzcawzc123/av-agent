...
- **桌面工作台（桌面版）**：PySide6 原生界面——对话工作台、项目与 BOM、产品库、模板库、模型提供商、招标改单、知识库一页直达；**双主题**（浅色/深色跟随系统，设置页可切换）；FastAPI 服务后台常驻，手机仍可经局域网访问
- **双对话模式**：专家模式（售前流程逐项澄清 → 确认 → 生成）与 **Agent 模式**（模型可自主多轮调用工具：查产品库/查项目/查文件/生成交付物/读写跨会话记忆，直到给出答复，轮次上限防失控）
- **全局长期记忆**：`data/MEMORY.md` 跨会话记录客户偏好、常用品牌等，专家与 Agent 模式对话自动注入；设置页可查看/编辑/清空
- **售前专家知识库**：首次启动预置 11 条专业速查文档（会议室系统组成、LED 点距速查、场景配置速查、扩声设计要点、招标偏离应对、报价结构等），对话意图解析自动检索注入；场景化追问按会议室/报告厅/展厅/指挥中心追加席位、层高、显示、视频会议等专家问题
...
| 桌面端 | PySide6 原生工作台（`desktop/`，`python -m desktop.main` 源码运行） |
...
1. 双击 `AVAgent.exe` 即启动，弹出 **PySide6 原生桌面工作台**（对话/项目/产品库/模板库/提供商/招标/知识库一页直达），服务在后台线程运行；
2. 首次运行在 exe 同级目录生成 `data\`（数据库 + 访问口令，重启不变）；
3. 终端窗口会显示本机/手机访问地址与访问口令；生成的文件默认下载到 `~\AVAgent 输出\`。
